...
object CMC_Claimant {
  ...
  val eligibility =
    doIf("${existingclaimcheck.exists()}") {
      group ("TX04_CMC_Eligibility_FindoutEligibilityPage") {
        exec (http ("TX04_CMC_Eligibility_FindoutEligibilityPage")
          .get ("/eligibility")
          .check (regex ("Find out if you can make a claim using this service")))
      }
    }
      .pause (MinThinkTime seconds, MaxThinkTime seconds) //eligibility questions : claim value get
    .group ("TX03_CMC_Eligibility_TotalAmountYouAreclaiming_GET") {
    ...
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  //eligibility questions : claim value post
  ...
  .headers(headers_202),*/
    ...
    exec (http ("TX08_CMC_Eligibility_ClaimType")
      .post (currentPageTemplate)
      .formParam (csrfParameter, csrfTemplate).formParam ("claimType", "PERSONAL_CLAIM").check (CurrentPageCheck.save).check (CsrfCheck.save).check (regex ("Do you have a postal address in the UK?")))
  }
    // .exitHereIfFailed
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
    .group ("TX09_CMC_Eligibility_ClaimantAddress") {
    exec (http ("TX09_CMC_Eligibility_ClaimantAddress").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("claimantAddress", "yes").check (CurrentPageCheck.save).check (CsrfCheck.save).check (regex ("Is your claim for a tenancy deposit?")))
  }
    // .exitHereIfFailed
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
    .group ("TX010_CMC_Eligibility_TenancyDeposit") {
    exec (http ("TX010_CMC_Eligibility_TenancyDeposit").post ("/eligibility/claim-is-for-tenancy-deposit").formParam (csrfParameter, csrfTemplate).formParam ("claimIsForTenancyDeposit", "no").check (CurrentPageCheck.save).check (regex ("Are you claiming against a government department?")))
  }
    // .exitHereIfFailed
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
    .group ("TX011_CMC_Eligibility_GovernmentDepartment") {
    exec (http ("TX011_CMC_Eligibility_GovernmentDepartment").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("governmentDepartment", "no").check (CurrentPageCheck.save).check (CsrfCheck.save).check (regex ("Do you believe the person you’re claiming against is 18 or over?")))
  }
    // .exitHereIfFailed
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
    .group ("TX012_CMC_Eligibility_DefendantAge") {
    exec (http ("TX012_CMC_Eligibility_DefendantAge").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("defendantAge", "yes").check (CurrentPageCheck.save).check (CsrfCheck.save).check (regex ("Are you 18 or over?")))
  }
    // .exitHereIfFailed//Do you believe the person you’re claiming against is 18 or over?
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
    .group ("TX013_CMC_Eligibility_Over18") {
    exec (http ("TX013_CMC_Eligibility_Over18").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("eighteenOrOver", "yes").check (CurrentPageCheck.save).check (CsrfCheck.save))
  }
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  // .check(regex("You can use this service")))
  // .exitHereIfFailed//Do you believe the person you’re claiming against is 18 or over?
    .group ("TX013_CMC_Eligibility_HelpwithFee") {
    exec (http ("TX013_CMC_Eligibility_HelpwithFee").post ("/eligibility/help-with-fees").formParam (csrfParameter, csrfTemplate).formParam ("helpWithFees", "no").check (CurrentPageCheck.save))
  }
    // .check(CsrfCheck.save))
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  ...
  val resolvingDispute =
    group ("TX015_CMC_ResolvingThisDispute_ResolvingThisDispute-GET") {
      exec (http ("TX015_CMC_ResolvingThisDispute_ResolvingThisDispute-GET").get ("/claim/resolving-this-dispute").check (CsrfCheck.save).check (CurrentPageCheck.save).check (regex ("Try to resolve the dispute")))
    }
      .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
    .group ("TX016_CMC_ResolvingThisDispute_ResolvingThisDispute-POST") {
    exec (http ("TX016_CMC_ResolvingThisDispute_ResolvingThisDispute-POST").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).check (regex ("Prepare your claim")))
  }
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  ...
  val completingClaim =
    group ("TX017_CMC_Completingclaim_CompletingClaim-GET") {
      exec (http ("TX017_CMC_Completingclaim_CompletingClaim-GET").get ("/claim/completing-claim").check (CsrfCheck.save).check (CurrentPageCheck.save).check (regex ("Get the details right")))
    }
      .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
    .group ("TX018_CMC_Completingclaim_CompletingClaim-POST") {
    exec (http ("TX018_CMC_Completingclaim_CompletingClaim-POST").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).check (regex ("Prepare your claim")))
  }
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
  /*
  below requests are for your details
   */
  val yourDetails =
      group ("TX019_CMC_YourDetail_PartyTypeSelection-GET") {
      exec (http ("TX019_CMC_YourDetail_PartyTypeSelection-GET").get ("/claim/claimant-party-type-selection").check (CsrfCheck.save).check (CurrentPageCheck.save).check (regex ("About you and this claim")))
    }
      .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
      .group ("TX020_CMC_YourDetail_PartyTypeSelection-POST") {
        exec (http ("TX020_CMC_YourDetail_PartyTypeSelection-POST").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("type", "individual").formParam ("saveAndContinue", "Save and continue").check (CsrfCheck.save).check (CurrentPageCheck.save).check (regex ("your details")))
      }
      .pause (MinThinkTime seconds, MaxThinkTime seconds)
     ...
     .headers(headers_202),
      ...
      exec (http ("TX021_CMC_YourDetail_YourDetails-POST").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("name", "mr vijay kumar").formParam ("address[postcodeLookup]", "TW3 3SD").formParam ("address[addressList]", """{"addressLines":["20, HIBERNIA GARDENS","",""],"townOrCity":"HOUNSLOW","postCode":"TW3 3SD"}""").formParam ("address[line1]", "20, HIBERNIA GARDENS").formParam ("address[line2]", "").formParam ("address[line3]", "").formParam ("address[city]", "HOUNSLOW").formParam ("address[postcode]", "TW3 3SD").formParam ("address[addressVisible]", "true").formParam ("address[addressSelectorVisible]", "false").formParam ("address[enterManually]", "false").formParam ("hasCorrespondenceAddress", "false").formParam ("correspondenceAddress[postcodeLookup]", "").formParam ("correspondenceAddress[addressList]", "").formParam ("correspondenceAddress[line1]", "").formParam ("correspondenceAddress[line2]", "").formParam ("correspondenceAddress[line3]", "").formParam ("correspondenceAddress[city]", "").formParam ("correspondenceAddress[postcode]", "").formParam ("correspondenceAddress[addressVisible]", "true").formParam ("correspondenceAddress[addressSelectorVisible]", "false").formParam ("correspondenceAddress[enterManually]", "false").formParam ("saveAndContinue", "Save and continue").check (CsrfCheck.save).check (CurrentPageCheck.save).check (regex ("What is your date of birth")))
    }
      .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
      .group ("TX022_CMC_YourDetail_DateOfBirth-POST") {
        exec (http ("TX022_CMC_YourDetail_DateOfBirth-POST").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("known", "true").formParam ("date[day]", "31").formParam ("date[month]", "3").formParam ("date[year]", "1980").formParam ("saveAndContinue", "Save and continue").check (CsrfCheck.save).check (CurrentPageCheck.save).check (regex ("Enter a phone number")))
      }
      .pause (MinThinkTime seconds, MaxThinkTime seconds)
      
      .group ("TX023_CMC_YourDetail_AddPhoneNumber-POST") {
        exec (http ("TX023_CMC_YourDetail_AddPhoneNumber-POST").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("number", "07123456789").formParam ("saveAndContinue", "Save and continue").check (regex ("Their details")))
      }
      .pause (MinThinkTime seconds, MaxThinkTime seconds)
  ...
  val TheirDetails =
    group ("TX024_CMC_TheirDetail_PartyTypeSelection_GET") {
      exec (http ("TX024_CMC_TheirDetail_PartyTypeSelection_GET").get ("/claim/defendant-party-type-selection").check (CsrfCheck.save).check (CurrentPageCheck.save).check (regex ("Who are you making the claim against")))
    }
      .pause (MinThinkTime seconds, MaxThinkTime seconds)
  
    .group ("TX025_CMC_TheirDetail_PartyTypeSelection_POST") {
    exec (http ("TX025_CMC_TheirDetail_PartyTypeSelection_POST").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("type", "company") // .formParam("saveAndContinue", "Save and continue")
    ...
    .group ("TX026_CMC_TheirDetail_TheirDetails_POST") {
      exec (http ("TX026_CMC_TheirDetail_TheirDetails_POST").post (currentPageTemplate).formParam (csrfParameter, csrfTemplate).formParam ("name", "perftest holiday company").formParam ("contactPerson", "asasasas").formParam ("address[postcodeLookup]", "TW3 3SD").formParam ("address[addressList]", """{"addressLines":["10, HIBERNIA GARDENS","",""],"townOrCity":"HOUNSLOW","postCode":"TW3 3SD"}""").formParam ("address[line1]", "10, HIBERNIA GARDENS").formParam ("address[line2]", "").formParam ("address[line3]", "").formParam ("address[city]", "HOUNSLOW").formParam ("address[postcode]", "TW3 3SD").formParam ("address[addressVisible]", "true").formParam ("address[addressSelectorVisible]", "false").formParam ("address[enterManually]", "false").formParam ("saveAndContinue", "Save and continue").check (CsrfCheck.save).check (CurrentPageCheck.save).check (regex ("Their email address")))
    }
    .pause (MinThinkTime seconds, MaxThinkTime seconds)
